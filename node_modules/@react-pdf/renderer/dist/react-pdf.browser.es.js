...
var formatter = function formatter(measureText, textAlign, callback, penalty) {
  var spaceWidth = measureText(' ');
  var hyphenWidth = measureText('-');
  var hyphenPenalty = penalty || (!textAlign || textAlign === 'justify' ? 100 : 600);
...
var lineBreaker = (function () {
  ...
  return function (Textkit) {
    ...
    return function () {
      ...
      KPLineBreaker.prototype.suggestLineBreak = function suggestLineBreak(glyphString, availableWidth, paragraphStyle) {
        var tolerance = this.tolerance;
        var width = this.measureWidth(glyphString);
        var nodes = formatter(width, paragraphStyle.align, callback, penalty)(glyphString);
        var breaks = [];

        // Try again with a higher tolerance if the line breaking failed.
        while (breaks.length === 0 && tolerance < TOLERANCE_LIMIT) {
          breaks = linebreak(nodes, [availableWidth], { tolerance: tolerance });
          tolerance += TOLERANCE_STEPS;
        }

        // Fallback to textkit default's linebreaking algorithm if K&P fails
        if (breaks.length === 0) {
          var fallback = fallbackLinebreaker.suggestLineBreak(glyphString, availableWidth, paragraphStyle);
          if (fallback) return fallback;

          // If fallback didn't worked, we split workd based on width
          var index = glyphString.glyphIndexAtOffset(availableWidth) - 1;
